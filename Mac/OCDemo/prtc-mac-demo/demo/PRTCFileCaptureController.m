...
#import "PRTCFileCaptureController.h"
NSString *const kYJFileVideoCapturerErrorDomain = @"org.webrtc.RTCFileVideoCapturer";


typedef NS_ENUM(NSInteger, YJFileVideoCapturerErrorCode) {
  YJFileVideoCapturerErrorCode_CapturerRunning = 2000,
  YJFileVideoCapturerErrorCode_FileNotFound
};

typedef NS_ENUM(NSInteger, YJFileVideoCapturerStatus) {
  YJFileVideoCapturerStatusNotInitialized,
  YJFileVideoCapturerStatusStarted,
  YJFileVideoCapturerStatusStopped
};
@implementation PRTCFileCaptureController
{
  AVAssetReader *_reader;
  AVAssetReaderTrackOutput *_outTrack;
  YJFileVideoCapturerStatus _status;
...
- (void)startCapturingFromFileNamed:(NSString *)nameOfFile
                            onError:(YJFileVideoCapturerErrorBlock)errorBlock {
  if (_status == YJFileVideoCapturerStatusStarted) {
    NSError *error =
        [NSError errorWithDomain:kYJFileVideoCapturerErrorDomain
                            code:YJFileVideoCapturerErrorCode_CapturerRunning
                        userInfo:@{NSUnderlyingErrorKey : @"Capturer has been started."}];

    errorBlock(error);
    return;
  } else {
    _status = YJFileVideoCapturerStatusStarted;
  }
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    if (!pathForFile) {
      ...
      NSError *error = [NSError errorWithDomain:kYJFileVideoCapturerErrorDomain
                                           code:YJFileVideoCapturerErrorCode_FileNotFound
...
- (void)setupReaderOnError:(YJFileVideoCapturerErrorBlock)errorBlock {
...
- (void)stopCapture {
  _status = YJFileVideoCapturerStatusStopped;
...
- (void)readNextBuffer {
  if (_status == YJFileVideoCapturerStatusStopped) {
...
- (void)testData:(CMSampleBufferRef)sampleBuffer {
    ...
    [self.delegate capturer:self didCaptureVideodata:data];
